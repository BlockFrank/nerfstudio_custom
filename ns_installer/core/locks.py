...
def normalized_current_pip_lines() -> list[str]:
    import subprocess
    import sys
    from ns_installer.core.bootstrap import run
...
def check_locks(lock_dir: Path, conda_exe: str | None = None) -> int:
    from ns_installer.core.bootstrap import run, which
...
def export_locks(lock_dir: Path, conda_exe: str | None = None, msvc_mode: str = "") -> int:
    from ns_installer.core.bootstrap import build_bootstrap_context, run, which, write_msvc_log
    ensure_dir(lock_dir)
    conda_exe = conda_exe or os.environ.get("CONDA_EXE") or which("conda") or which("conda.bat")
    platform_tag = current_platform_guess()
