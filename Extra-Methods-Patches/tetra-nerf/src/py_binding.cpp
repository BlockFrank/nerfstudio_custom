...
struct PyTetrahedraTracer {
   public:
    ...
    py::dict trace_rays(const torch::Tensor &ray_origins,
        ...
        const auto num_visited_cells =
            torch::zeros({(long)num_rays}, torch::device(device).dtype(torch::kInt32));
        const auto visited_cells =
            torch::zeros({(long)num_rays, (long)max_ray_triangles}, torch::device(device).dtype(torch::kInt32));
        const auto barycentric_coordinates =
            torch::zeros({(long)num_rays, (long)max_ray_triangles, 2, 3}, torch::device(device).dtype(torch::kFloat32));
        const auto hit_distances =
            torch::zeros({(long)num_rays, (long)max_ray_triangles, 2}, torch::device(device).dtype(torch::kFloat32));
        const auto vertex_indices =
            torch::zeros({(long)num_rays, (long)max_ray_triangles, 4}, torch::device(device).dtype(torch::kInt32));
    ...
    py::dict trace_rays_triangles(const torch::Tensor &ray_origins,
        ...
        const auto num_visited_triangles =
            torch::zeros({(long)num_rays}, torch::device(device).dtype(torch::kInt32));
        const auto visited_triangles =
            torch::zeros({(long)num_rays, (long)max_ray_triangles}, torch::device(device).dtype(torch::kInt32));
        const auto barycentric_coordinates =
            torch::zeros({(long)num_rays, (long)max_ray_triangles, 2}, torch::device(device).dtype(torch::kFloat32));
        const auto hit_distances =
            torch::zeros({(long)num_rays, (long)max_ray_triangles}, torch::device(device).dtype(torch::kFloat32));
        const auto vertex_indices =
            torch::zeros({(long)num_rays, (long)max_ray_triangles, 3}, torch::device(device).dtype(torch::kInt32));
    ...
    py::dict find_tetrahedra(const torch::Tensor &positions) {
        ...
        const auto barycentric_coordinates =
            torch::zeros(shape, torch::device(device).dtype(torch::kFloat32));

        shape.pop_back();
        shape.push_back(4);
        const auto vertex_indices =
            torch::zeros(shape, torch::device(device).dtype(torch::kInt32));
    ...
    void load_tetrahedra(const torch::Tensor &xyz, const torch::Tensor &cells) {
    ...
    py::dict find_visited_cells(const torch::Tensor &num_visited_cells,
        ...
        TORCH_CHECK(
            distances.dim() == 2 && distances.size(0) == num_rays,
            "distances must be of [num_rays, num_samples_per_ray] shape");
        ...
        const auto mask =
            torch::full({(long)num_rays, (long)num_samples_per_ray}, 0, torch::device(device).dtype(torch::kBool));
        const auto matched_cells =
            torch::full({(long)num_rays, (long)num_samples_per_ray}, -1, torch::device(device).dtype(torch::kInt32));
        const auto barycentric_coordinates_out =
            torch::zeros({(long)num_rays, (long)num_samples_per_ray, 3}, torch::device(device).dtype(torch::kFloat32));
        const auto vertex_indices_out =
            torch::full({(long)num_rays, (long)num_samples_per_ray, 4}, -1, torch::device(device).dtype(torch::kInt32));

        find_matched_cells(
            num_rays,
            num_samples_per_ray,
            visited_cells.size(1),
            reinterpret_cast<uint4 *>(tetrahedra_cells.value().data_ptr()),
            reinterpret_cast<unsigned int *>(num_visited_cells.data_ptr()),
            reinterpret_cast<unsigned int *>(visited_cells.data_ptr()),
            reinterpret_cast<float2 *>(hit_distances.data_ptr()),
...
torch::Tensor py_triangulate(const torch::Tensor &points) {
    ...
    if (cells.size() >= static_cast<size_t>(std::numeric_limits<int>::max())) {
...
torch::Tensor py_interpolate_values(const torch::Tensor &vertex_indices,
    ...
    TORCH_CHECK(barycentric_coordinates.size(-1) + 1 == vertex_indices.size(-1),
                "barycentric_coordinates must have the same last dimension as vertex_indices - 1");
    TORCH_CHECK(field.dtype() == torch::kFloat32, "field must be a tensor of type float32");

    const uint32_t interpolation_dim = static_cast<uint32_t>(vertex_indices.size(-1));
    const uint32_t num_values = static_cast<uint32_t>(vertex_indices.numel() / interpolation_dim);
    const uint32_t field_dim = static_cast<uint32_t>(field.size(0));
    const uint32_t num_vertices = static_cast<uint32_t>(field.size(-1));
    ...
    call_dynamic_interpolate_values(
        ...
        num_values,
        field_dim,
        reinterpret_cast<uint32_t *>(vertex_indices.data_ptr()),
...
torch::Tensor py_interpolate_values_backward(const torch::Tensor &vertex_indices,
    ...
    const auto grad_field_out =
        torch::zeros({(long)field_dim, (long)num_vertices}, torch::device(grad_in.device()).dtype(grad_in.dtype()));

    auto grad_in_moved = grad_in.moveaxis(-1, 0).contiguous();
    ...
    call_dynamic_interpolate_values_backward(
        ...
        reinterpret_cast<uint32_t *>(vertex_indices.data_ptr()),
        reinterpret_cast<float *>(barycentric_coordinates.data_ptr()),
        reinterpret_cast<float *>(grad_in_moved.data_ptr()),
...
torch::Tensor py_gather_uint32(const torch::Tensor &self, const uint32_t dim, const torch::Tensor &index) {
    ...
    AT_DISPATCH_FLOATING_TYPES(self.scalar_type(), "gather_uint32", [&] {
        gather_uint32<scalar_t>(
            static_cast<uint32_t>(self.numel()),
...
void py_scatter_ema_uint32(const torch::Tensor &self,
                           const uint32_t dim,
