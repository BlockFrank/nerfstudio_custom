...
extern "C" {
    extern char ptx_code_file[];
    extern char ptx_code_file_find_tetrahedra[];
    extern char ptx_code_file_triangles[];
}
...
class TetrahedraStructure {
   public:
    ...
    const uint3 *triangle_indices() const { return triangle_indices_; }
    const uint2 *triangle_tetrahedra() const { return triangle_tetrahedra_; }
    bool defined() const { return gas_handle_ != 0; }
...
class TraceRaysPipeline {
   ...
   private:
    ...
    static std::string load_ptx_data() {
        return std::string(ptx_code_file);
    }
};
...
class TraceRaysTrianglesPipeline {
   ...
   private:
    ...
    static std::string load_ptx_data() {
        return std::string(ptx_code_file_triangles);
    }
};
...
class FindTetrahedraPipeline {
   ...
   private:
    ...
    static std::string load_ptx_data() {
        return std::string(ptx_code_file_find_tetrahedra);
    }
};
...
class TetrahedraTracer {
   public:
    ...
    TetrahedraTracer(TetrahedraTracer &&other) noexcept;
...
template <uint32_t interpolation_dim>
void interpolate_values_backward(
    const uint32_t num_vertices,
    const uint32_t num_values,
    const uint32_t field_dim,
    const uint32_t *vertex_indices,
...
template <typename scalar_t>
void scatter_ema_uint32(
    const uint32_t num_result,
    const uint32_t num_indices,
    const uint32_t *indices,
