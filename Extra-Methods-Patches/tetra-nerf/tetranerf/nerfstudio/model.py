...
@dataclass
class TetraPointFilterConfig:
    enabled: bool = True

    # Conservative defaults first
    radial_keep_quantile: float = 0.995   # keep 99.5%
    z_keep_min_quantile: float = 0.002    # drop bottom 0.2%
    z_keep_max_quantile: float = 1.0      # keep full top side for now

    # Optional light deduplication
    voxel_size: Optional[float] = None    # e.g. 5e-5 or 1e-4 after transform
    max_points: Optional[int] = None      # e.g. 120000 if needed later

    verbose: bool = True


def _apply_transform_and_scale(
    points_xyz: torch.Tensor,
    dataparser_transform: torch.Tensor,
    dataparser_scale: float,
) -> torch.Tensor:
    pts = points_xyz.float()

    tf = dataparser_transform.float()
    if tf.shape == (3, 4):
        R = tf[:, :3]
        t = tf[:, 3]
    elif tf.shape == (4, 4):
    ...
    pts = pts @ R.T + t
    pts = pts * float(dataparser_scale)
    return pts


def _quantile_str(x: torch.Tensor, qs=(0.0, 0.5, 0.95, 0.99, 1.0)) -> str:
    vals = torch.quantile(x.float().cpu(), torch.tensor(qs, dtype=torch.float32))
    return ", ".join([f"q{int(q*1000)/10:g}={v.item():.6g}" for q, v in zip(qs, vals)])


def _deduplicate_by_voxel(points: torch.Tensor, voxel_size: float) -> torch.Tensor:
    if voxel_size is None or voxel_size <= 0:
        return points

    device = points.device
    scaled = torch.floor(points / voxel_size).to(torch.int64)
    _, unique_idx = torch.unique(scaled, dim=0, return_index=True)
    unique_idx = unique_idx.sort().values
    return points[unique_idx.to(device)]

def _filter_points_for_tetra(
    points_xyz: torch.Tensor,
    dataparser_transform: torch.Tensor,
    dataparser_scale: float,
    cfg: TetraPointFilterConfig,
) -> Tuple[torch.Tensor, torch.Tensor, dict]:
    # IMPORTANT TEST:
    # metadata points may already be in dataparser/model space
    pts = points_xyz.float()

    stats = {
        "num_points_raw": int(pts.shape[0]),
    }

    kept_idx = torch.arange(pts.shape[0], dtype=torch.long, device=pts.device)

    if not cfg.enabled:
        stats["num_points_final"] = int(pts.shape[0])
        return pts, kept_idx, stats

    keep = torch.ones(pts.shape[0], dtype=torch.bool, device=pts.device)

    # 1) radial tail pruning
    center = pts.mean(dim=0, keepdim=True)
    radial = torch.linalg.norm(pts - center, dim=-1)
    if cfg.radial_keep_quantile < 1.0:
        radial_q = torch.quantile(radial, cfg.radial_keep_quantile)
        keep &= radial <= radial_q
    else:
        radial_q = radial.max()

    # 2) asymmetric z-tail pruning
    z = pts[:, 2]
    z_lo = torch.quantile(z, cfg.z_keep_min_quantile) if cfg.z_keep_min_quantile > 0 else z.min()
    z_hi = torch.quantile(z, cfg.z_keep_max_quantile) if cfg.z_keep_max_quantile < 1.0 else z.max()
    keep &= (z >= z_lo) & (z <= z_hi)

    pts_filtered = pts[keep]
    kept_idx = kept_idx[keep]

    # 3) optional voxel dedup
    before_voxel = pts_filtered.shape[0]
    if cfg.voxel_size is not None and cfg.voxel_size > 0:
        scaled = torch.floor(pts_filtered / cfg.voxel_size).to(torch.int64)
        _, unique_idx = torch.unique(scaled, dim=0, return_index=True)
        unique_idx = unique_idx.sort().values.to(pts_filtered.device)
        pts_filtered = pts_filtered[unique_idx]
        kept_idx = kept_idx[unique_idx]
    after_voxel = pts_filtered.shape[0]

    # 4) optional hard cap
    if cfg.max_points is not None and pts_filtered.shape[0] > cfg.max_points:
        idx = torch.randperm(pts_filtered.shape[0], device=pts_filtered.device)[: cfg.max_points]
        pts_filtered = pts_filtered[idx]
        kept_idx = kept_idx[idx]

    stats.update({
        "num_points_after_mask": int(before_voxel),
        "num_points_after_voxel": int(after_voxel),
        "num_points_final": int(pts_filtered.shape[0]),
        "radial_keep_quantile": float(cfg.radial_keep_quantile),
        "z_keep_min_quantile": float(cfg.z_keep_min_quantile),
        "z_keep_max_quantile": float(cfg.z_keep_max_quantile),
        "z_range_after": (
            float(pts_filtered[:, 2].min().item()),
            float(pts_filtered[:, 2].max().item()),
        ) if pts_filtered.numel() > 0 else None,
        "radial_stats_after": _quantile_str(
            torch.linalg.norm(pts_filtered - pts_filtered.mean(dim=0, keepdim=True), dim=-1)
        ) if pts_filtered.numel() > 0 else "empty",
    })

    if cfg.verbose:
        print("Filtered tetra points:")
        for k, v in stats.items():
            print(f"    {k}: {v}")

    return pts_filtered, kept_idx, stats
...
class TetrahedraNerf(Model):
    ...
    def _load_points_from_metadata(self, points3D_xyz, points3D_rgb=None, **kwargs):
        ...
        # Safety
        if self.dataparser_transform is None or self.dataparser_scale is None:
            raise RuntimeError(
                "Missing dataparser transform/scale. Use TetrahedraNerfPipeline."
            )

        if not isinstance(points3D_xyz, torch.Tensor):
            points3D_xyz = torch.tensor(points3D_xyz, dtype=torch.float32)
        points3D_xyz = points3D_xyz.float().cpu()

        # ==============================
        # 🔥 PRESET SWITCH (manual)
        # ==============================
        PRESET = os.environ.get("TETRA_FILTER_PRESET", "A").upper()  # change: "OFF", "A", "B", "C"

        if PRESET == "OFF":
            tetra_filter_cfg = TetraPointFilterConfig(enabled=False)

        elif PRESET == "A":
            tetra_filter_cfg = TetraPointFilterConfig(
                enabled=True,
                radial_keep_quantile=1.0,
                z_keep_min_quantile=0.0001,
                z_keep_max_quantile=1.0,
                voxel_size=None,
                verbose=True,
            )
        ...
        # 🔥 FILTER (IMPORTANT)
        # ==============================
        tetrahedra_vertices, kept_idx, stats = _filter_points_for_tetra(
            points_xyz=points3D_xyz,
            dataparser_transform=self.dataparser_transform.cpu(),
            dataparser_scale=float(self.dataparser_scale),
            cfg=tetra_filter_cfg,
        )

        # ==============================
        # 🔥 TRIANGULATE FILTERED
        # ==============================
        tetrahedra_cells = triangulate(tetrahedra_vertices.contiguous()).int()

        # ==============================
        # REGISTER
        # ==============================
        num_vertices = len(tetrahedra_vertices)

        self.config.num_tetrahedra_vertices = num_vertices
        self.config.num_tetrahedra_cells = len(tetrahedra_cells)

        self.register_buffer("tetrahedra_vertices", tetrahedra_vertices)
        self.register_buffer("tetrahedra_cells", tetrahedra_cells.to(torch.int32))

        self.register_parameter(
            "tetrahedra_field",
            nn.Parameter(
                torch.empty(
                    (self.config.field_dim, num_vertices),
                    dtype=torch.float32,
                )
            ),
        )
        ...
        # COLOR INIT (FIXED)
        # ==============================
        if self.config.initialize_colors and points3D_rgb is not None:
            assert points3D_rgb.dtype == torch.uint8
            assert points3D_rgb.shape[0] == points3D_xyz.shape[0]

            filtered_rgb = points3D_rgb[kept_idx.cpu()]
            colors = filtered_rgb.float().to(self.tetrahedra_field.device) * 2.0 / 255.0 - 1.0
            self.tetrahedra_field.data[1:4, :] = colors[:, :3].T
            self.tetrahedra_field.data[0, :] = 1.0
        else:
            # still keep the density-compatible init
            self.tetrahedra_field.data[0, :] = 1.0

        # ==============================
        # DEBUG PRINTS (CRITICAL)
        # ==============================
        CONSOLE.print("\nFiltered tetra stats:")
        for k, v in stats.items():
            CONSOLE.print(f"    {k}: {v}")

        CONSOLE.print("\nTetrahedra initialized from dataparser:")
        CONSOLE.print(f"    Num points (raw): {len(points3D_xyz)}")
        CONSOLE.print(f"    Num points (filtered): {len(self.tetrahedra_vertices)}")
