...
class ExportGaussianSplat(Exporter):
    ...
    def main(self) -> None:
        ...
        # Accept any splatfacto-like model (including your custom one)
        if not hasattr(model, "means") or not hasattr(model, "scales"):
            raise TypeError(f"Model {type(model).__name__} is not splatfacto-compatible")
