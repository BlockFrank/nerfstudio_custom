...
from pynerf.pynerf_constants import (
    RGB,
    WEIGHT,
    TRAIN_INDEX,
    DEPTH,
...
class RandomSubsetDataManagerConfig(DataManagerConfig):
    ...
    dataparser: AnnotatedDataParserUnion = field(default_factory=MulticamDataParserConfig)
    ...
    """
    Range for patch tile size ratios.

    IMPORTANT:
    This must be float-based. Older code sometimes annotated this as Tuple[int, int]
    while using a default like (0.05, 0.5), which breaks newer Tyro versions when
    generating the CLI.
    """
...
class RandomSubsetDataManager(DataManager):
    ...
    def __init__(
        ...
        self.train_camera_optimizer = self.config.camera_optimizer.setup(
            num_cameras=self.train_dataparser_outputs.cameras.size,
            device=self.device,
        )
        ...
        self.eval_camera_optimizer = self.config.camera_optimizer.setup(
            num_cameras=self.eval_dataparser_outputs.cameras.size,
            device=self.device,
        )
        self.eval_ray_generator = RayGenerator(
            self.eval_dataparser_outputs.cameras.to(self.device),
    ...
    def _set_train_loader(self):
        batch_size = self.config.train_num_rays_per_batch // max(self.world_size, 1)

        if self.world_size > 1:
            self.train_sampler = DistributedSampler(
                self.train_batch_dataset,
                self.world_size,
                self.local_rank,
            )
    ...
    def _set_eval_batch_loader(self):
        batch_size = self.config.eval_num_rays_per_batch // max(self.world_size, 1)

        if self.world_size > 1:
            self.eval_sampler = DistributedSampler(
                self.eval_batch_dataset,
                self.world_size,
                self.local_rank,
            )
            assert self.config.eval_num_rays_per_batch % self.world_size == 0
            self.eval_batch_dataloader = DataLoader(
                self.eval_batch_dataset,
                batch_size=batch_size,
    ...
    def next_eval_image(self, step: int) -> Tuple[int, RayBundle, Dict]:
        ...
        if metadata.train_index is not None:
            ray_bundle.metadata[TRAIN_INDEX] = torch.full_like(
                ray_bundle.camera_indices,
                metadata.train_index,
                dtype=torch.int64,
            )
        if metadata.weight is not None:
            batch[WEIGHT] = torch.full_like(
                ray_bundle.camera_indices,
                metadata.weight,
                dtype=torch.float32,
            )
    ...
    @staticmethod
    def transfer_train_index(ray_bundle: RayBundle, batch: Dict) -> None:
